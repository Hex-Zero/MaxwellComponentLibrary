...
export const sizeStyles: Record<ButtonSize, ReturnType<typeof css>> = {
  '2xs': css`font-size: 10px; padding: 2px 6px;`,
...
export const variantStyles = (t: ThemeWithNeo, v: ButtonVariant) => {
  ...
  switch (v) {
    case 'primary':
      return css`${dynamic(c.primary, `&:hover{background:${c.primaryHover}} &:active{background:${c.primaryActive}}`)}`;
    case 'secondary':
      return css`${dynamic(c.secondary, `&:hover{background:${c.secondaryHover}} &:active{background:${c.secondaryActive}}`)}`;
    case 'ghost':
      return css`${base} background:transparent; &:hover{background:${c.ghostHover}}`;
    case 'danger':
      return css`${dynamic(c.danger, `&:hover{filter:brightness(0.9)}`)}`;
    case 'success':
      return css`${dynamic(c.success, `&:hover{filter:brightness(0.9)}`)}`;
    case 'outline':
...
export const StyledButton = styled.button<Required<Pick<ButtonProps, 'fullWidth' | 'size' | 'variant' | 'shape'>>>`
  ...
  &[aria-pressed='true'] { filter: brightness(0.95); }
  &:disabled, &[aria-disabled='true'] { opacity: ${(p) => p.theme.opacities.disabled}; cursor: not-allowed; box-shadow: none; }
  & .btn-label { display: inline-flex; align-items: center; }
  &[aria-busy='true'] { pointer-events: none; }
  /* icon-only adjustment */
  &[aria-label][role='button'], &[aria-label]:not(:has(.btn-label)) {
