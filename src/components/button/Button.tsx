...
function useConfirm(requireConfirm: boolean | undefined, confirmTimeout: number, onConfirm?: React.MouseEventHandler<ClickEl>) {
...
export const Button: React.FC<ButtonProps> = ({
  ...
  const accessibleLabel = iconOnly && leftIcon && typeof children === 'string' ? { 'aria-label': children, title: children } : {};
  ...
  const loadingNode = loading && loadingStyle === 'spinner' ? <LoadingSpinner aria-hidden="true" /> : null;
  ...
  >
    ...
    {showLeft}
    {!iconOnly && !skeleton && <span className="btn-label">{confirming ? confirmLabel : children}</span>}
    {showRight}
    {showProgress && <span className="btn-progress" aria-hidden="true"><span className="bar" style={{ transform: `scaleX(${Math.min(Math.max(progress ?? 0, 0), 1)})` }} /></span>}
  </StyledButton>;
};
