      ...
      {(['light', 'dark'] as const).map((tone) => (
        <div key={tone} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-6 gap-4">
            ...
            <NeoButton variant="raised" shape="rounded" tone={tone}>
              rounded
            </NeoButton>
            <NeoButton variant="raised" shape="pill" tone={tone}>
              pill
            </NeoButton>
            <NeoButton variant="raised" shape="circle" aria-label="circle" tone={tone}>
              ○
            </NeoButton>
            <NeoIconButton variant="raised" tone={tone} label="icon">
              ★
            </NeoIconButton>
            <NeoButton variant="inset" shape="circle" size="lg" aria-pressed tone={tone}>
              ⬤
            </NeoButton>
            <NeoButton variant="raised" shape="pill" disabled tone={tone}>
              disabled
            </NeoButton>
          </div>
        </div>
