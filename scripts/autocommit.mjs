...
try {
  ...
  if (!postAdd) {
    globalThis.console?.log('No staged changes');
    globalThis.process?.exit(0);
  }
  ...
  try {
    ...
    globalThis.console?.log('Nothing to commit');
  }
  try {
    execSync('git push origin HEAD', { stdio: 'inherit' });
  } catch (e) {
    globalThis.console?.error('Push failed (non-fatal):', e?.message || e);
  }
} catch (err) {
  globalThis.console?.error(
    'autocommit failed (non-fatal):',
